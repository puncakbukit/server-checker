...
//            mapper.configure(DeserializationFeature.USE_LONG_FOR_INTS, true);
			...
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat(SteemJConfig.getInstance()
					.getDateTimePattern());
			simpleDateFormat.setTimeZone(TimeZone.getTimeZone(SteemJConfig.getInstance()
					.getTimeZoneId()));

			mapper.setDateFormat(simpleDateFormat);
			mapper.setTimeZone(TimeZone.getTimeZone(SteemJConfig.getInstance()
					.getTimeZoneId()));
	...
	public void initializeNewClient() throws SteemCommunicationException {
		...
		Pair<URI, Boolean> endpoint = SteemJConfig.getInstance()
				.getNextEndpointURI(numberOfConnectionTries);
		...
		if (endpoint.getLeft()
			...
			throw new InvalidParameterException(
					"No client implementation for the following protocol available: " + endpoint.getLeft()
							.getScheme()
							.toLowerCase());
		}
	}
	...
	public <T> List<T> performRequest(JsonRPCRequest requestObject, Class<T> targetClass)
		...
		try {
			...
			if (rawJsonResponse.isError()) {
				...
				JavaType expectedResultType = mapper.getTypeFactory()
